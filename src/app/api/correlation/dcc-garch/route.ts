...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (symbolsParam) {
      ...
      symbols = Object.keys(SYMBOL_MAP).slice(0, 6);
    }

    const returnsData = await getMultiPriceHistory(symbols, days, '1d');
    const dccResult = fitDCCGARCH(returnsData);

    return NextResponse.json({ success: true, data: dccResult });
  } catch (error) {
    console.error('[API /correlation/dcc-garch] Error:', error);
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : 'Failed to compute DCC-GARCH' },
      { status: 500 }
    );
  }
}
